...
wl1, wl2 = 680, 800
wavs = np.genfromtxt(fpaths[0], delimiter=';', skip_header=33, skip_footer=1,
...
for r, lab in zip(refl, labels):
    ax.plot(wavs, r, label=lab)
...
fit_results = [opt.curve_fit(mim.lorentz, wavs[i1:i2], r[i1:i2], popt_0,
                             refl_err*np.ones(i2-i1), True, method='lm')
               for r in refl]
